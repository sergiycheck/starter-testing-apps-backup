...
class VehicleFactory {
  private static vehicleFlyweights: Map<VehicleType, VehicleFlyweight> = new Map<
    VehicleType,
    VehicleFlyweight
  >();
...
for (const [key, value] of typedEntries(process.memoryUsage())) {
  console.log(`    ${Math.round(value / (1024 * 1024))}MB of ${key.toString()}`);
}
